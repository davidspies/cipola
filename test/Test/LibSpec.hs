...
spec = describe "modRoot" $ do
  it "should return modulo square roots" $
    property $
      \(Positive n) a ->
  ...
  it "should return a minimal number of roots" $
    property $
      \(Positive n) a ->
        let n' = fromInteger n
            (xs, _m) = modRoot a n'
         in length xs <= 2 ^ (length (primeDecomposition n') + 1)
  it "should return a modulus which is a factor of the input modulus" $
    property $
      \(LargePrimeVector n) a ->
        let (_, m) = modRoot a n in toInteger m `divides` toInteger n
  it "should handle very large moduli" $
    property $
